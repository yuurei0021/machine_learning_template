"""
Gaussian Naive Bayes + Original Data Reference
Experiment: 20260327_05_gnb_orig_data_reference

201特徴量（元データ参照FE + Nested TE）でGNBを実行。
FEパイプラインは20260323_01_orig_data_referenceと同一。
"""
...
def apply_target_encoding(X_tr, y_tr, X_val, X_te,
    ...
    for col in ngram_columns:
        ...
        te_col = f"TE_ng_{col}"
        X_val[te_col] = pd.to_numeric(X_val[col].astype(str).map(ng_te), errors="coerce").fillna(0.5).astype("float32")
        X_te[te_col] = pd.to_numeric(X_te[col].astype(str).map(ng_te), errors="coerce").fillna(0.5).astype("float32")

    te_mean = TargetEncoder(cv=inner_folds, shuffle=True, smooth="auto", target_type="binary", random_state=seed)
    te_mean_cols = [f"TE_{col}" for col in te_columns]
    X_tr[te_mean_cols] = te_mean.fit_transform(X_tr[te_columns], y_tr)
    X_val[te_mean_cols] = te_mean.transform(X_val[te_columns])
    X_te[te_mean_cols] = te_mean.transform(X_te[te_columns])

    drop_cols = [c for c in te_columns + ngram_columns if c in X_tr.columns]
    X_tr.drop(columns=drop_cols, inplace=True)
    X_val.drop(columns=drop_cols, inplace=True)
    X_te.drop(columns=drop_cols, inplace=True)
...
def main():
    ...
    print("=" * 60)
    print("Gaussian Naive Bayes + Original Data Reference (201 features)")
    print("=" * 60)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        X_te = X_te.fillna(0).astype("float32")
        ...
        test_preds += model.predict_proba(X_te)[:, 1] / config.N_SPLITS
