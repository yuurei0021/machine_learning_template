"""
Hill Climbing Ensemble v4
Experiment: 20260327_10_hillclimb_ensemble_v4

v3 + Code1方式に書き換え:
  - ランク正規化（rank01）
  - GPU並列評価（CuPy）
  - 同一モデルの複数回選択を許容
  - GPU近似AUC（探索中）+ CPU厳密AUC（最終確認）
"""
...
FIRST = None  # 開始モデルを強制指定する場合は名前を入れる

# ============================================================================
# Helpers
# ============================================================================
def rank01(x: np.ndarray) -> np.ndarray:
    """予測値をグローバルランクで [0, 1] に正規化する"""
...
def auc_cpu(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """CPU上での厳密AUC計算"""
    return float(roc_auc_score(y_true, y_pred))


def multiple_roc_auc_scores(y_gpu: cp.ndarray, preds_gpu: cp.ndarray) -> cp.ndarray:
    """
    GPU上で複数の予測列を一括近似AUC評価する。
    タイ処理は省略（探索用途なので十分な精度）。
    preds_gpu: shape (N, n_weights)
    """
    n_pos = cp.sum(y_gpu)
...
# ============================================================================
# Hill Climbing（Code1方式）
# ============================================================================
def hill_climbing_ensemble(oof_dict, y_true):

    names   = list(oof_dict.keys())
    K       = len(names)
    x_train = np.column_stack([oof_dict[n] for n in names])  # (N, K)
    N       = x_train.shape[0]
    ...
    if FIRST is None:
        ...
        if FIRST not in names:
            raise ValueError(f"FIRST='{FIRST}' not found.")
        start_idx = names.index(FIRST)

    print(f"Starting from: {names[start_idx]} (AUC={single_aucs[start_idx]:.6f})")

    # --- GPU転送 ---
    x_gpu = cp.asarray(x_train, dtype=cp.float32)          # (N, K)
    y_gpu = cp.asarray(y_true,  dtype=np.int8)               # (N,)
    ww    = cp.arange(start_w, stop_w + 1e-9, step_w,
                      dtype=cp.float32)                      # (W,)

    best_ensemble = x_gpu[:, start_idx].copy()              # (N,)
    best_score    = float(single_aucs[start_idx])

    selected_indices = [start_idx]   # 選択されたモデルのインデックス列
    weights_step     = []            # 各ステップで採用した重み
    ...
    for it in range(1_000_000):
        ...
        # 現アンサンブルに (1-w) をかけたベース: shape (N, W)
    ...
    dfw = (
        pd.DataFrame({
            "model":  [names[i] for i in selected_indices],
            "weight": wgt,
        })
...
def main():
    print("=" * 70)
    print("Hill Climbing Ensemble v4 (GPU / rank01 / re-selection allowed)")
    print("=" * 70)
    ...
    dfw.to_csv(PRED_DIR / "ensemble_weights.csv", index=False)
