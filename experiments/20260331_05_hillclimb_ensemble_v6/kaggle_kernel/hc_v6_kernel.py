"""
Hill Climbing Ensemble v6 (Kaggle GPU)
...
- Removed lgbm_optuna (fold overfit suspected)
"""
...
# Config
# ============================================================================
DATA_DIR = Path("/kaggle/input/datasets/nyhalcyon/hc-v4-oof-predictions")
...
MODELS = {
    ...
    "stacking_lgbm":            "20260331_01_stacking_lgbm",
    "stacking_lgbm_optuna":     "20260331_02_stacking_lgbm_optuna",
    "stacking_xgb":             "20260331_03_stacking_xgb",
}
...
# ============================================================================
# Load predictions (from Kaggle dataset)
# ============================================================================
def load_predictions():
    ...
    if DATA_DIR.exists():
        ...
        print(f"  Files in DATA_DIR ({len(files)}): {files[:5]}...")
    else:
        ...
        for d in os.listdir('/kaggle/input/'):
            ...
            if os.path.isdir(full):
                ...
                if any('oof' in f for f in contents):
                    print(f"  Found OOF files in: {full}")
                    print(f"    Files: {contents[:5]}...")

    for short_name, exp_name in MODELS.items():
        oof_path  = DATA_DIR / f"oof_{exp_name}.csv"
        test_path = DATA_DIR / f"test_proba_{exp_name}.csv"

        if not oof_path.exists():
            print(f"  SKIP {short_name}: {oof_path.name} not found")
            continue
        ...
        if test_path.exists():
            ...
            print(f"  WARN {short_name}: {test_path.name} not found")
...
def main():
    print("=" * 70)
    print("Hill Climbing Ensemble v6 (GPU / rank01 / no lgbm_optuna)")
    print("=" * 70)
