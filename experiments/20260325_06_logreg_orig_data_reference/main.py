"""
Logistic Regression + Original Data Reference Features
Experiment: 20260325_06_logreg_orig_data_reference

目的: 9つの特徴量グループ + Nested TE + StandardScaler + LogisticRegression
"""
...
class Config:
    ...
    # Linear model to use: "logreg" or "ridge"
    MODEL_TYPE = "logreg"
...
def main():
    print("=" * 80)
    model_name = "Logistic Regression" if config.MODEL_TYPE == "logreg" else "Ridge Regression"
    print(f"{model_name} + Original Data Reference Features")
    print("=" * 80)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        if config.MODEL_TYPE == "logreg":
            ...
            model.fit(X_tr_s, y[tr_idx])
            val_proba = np.clip(model.predict(X_va_s), 0, 1)
            te_proba = np.clip(model.predict(X_te_s), 0, 1)
