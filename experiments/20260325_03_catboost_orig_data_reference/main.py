"""
Original Data Reference Distribution + CatBoost
Experiment: 20260325_03_catboost_orig_data_reference

目的: 実験01の元データ参照特徴量をCatBoostに適用し、アンサンブル候補を作成
アプローチ: 9つの特徴量グループ + Nested Target Encoding + CatBoost (5-Fold Stratified CV)
CatBoostはカテゴリカル特徴量をネイティブに扱えるが、TE特徴量との整合性のため
XGB/LGBMと同じLabel Encoding + TE pipeline を使用
"""
...
# ============================================================================
# Target Encoding (same as 20260323_01)
# ============================================================================
...
# ============================================================================
# Training (CatBoost with nested TE)
# ============================================================================
def train_with_cv(X, y, X_test_base, te_columns, ngram_columns):
    print(f"\n{'='*80}")
    print(f"Starting {config.N_SPLITS}-Fold Cross Validation (CatBoost)")
    print(f"{'='*80}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        test_preds_sum += model.predict_proba(X_te)[:, 1]
        ...
        fold_scores.append({
            ...
            "accuracy": fold_acc, "best_iteration": model.best_iteration_,
        })
        print(f"  AUC: {fold_auc:.6f}, LogLoss: {fold_logloss:.6f}, Accuracy: {fold_acc:.6f}, Best iter: {model.best_iteration_}")
...
def plot_feature_importance(models, save_path, top_n=30):
...
def save_models(models):
    ...
    for fold, model in enumerate(models):
        model.save_model(str(config.MODEL_DIR / f"fold_{fold}.cbm"))
...
def main():
    print("=" * 80)
    print("Original Data Reference Distribution + CatBoost")
    print("=" * 80)
    ...
    plot_feature_importance(models, config.BASE_DIR / "feature_importance.png")
