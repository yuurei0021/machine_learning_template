...
YDFはWindows非対応のためKaggle CPUで実行。
Discussion 003の知見: max_depth=2, BEST_FIRST_GLOBAL, categorical_algorithm=RANDOM
...
def prepare_features(train_df, test_df):
    ...
    # YDF handles categoricals natively, no need for Label Encoding
    exclude_cols = {TARGET_COL, ID_COL, "Churn_binary"}
    feature_cols = [c for c in train_df.columns if c not in exclude_cols]
    return train_df[feature_cols + [TARGET_COL]].copy(), test_df[feature_cols].copy(), feature_cols
...
def main():
    ...
    print(f"  max_depth={YDF_PARAMS['max_depth']}, growing={YDF_PARAMS['growing_strategy']}")
    print("=" * 60)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, y_binary)):
        ...
        va_data = train_df.iloc[va_idx][feature_cols + [TARGET_COL]].copy()
        te_data = test_df[feature_cols].copy()

        learner = ydf.GradientBoostedTreesLearner(**YDF_PARAMS)
