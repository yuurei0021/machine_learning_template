"""
LightGBM Baseline
Experiment: 20260313_05_lightgbm_baseline

目的: LightGBMによるベースライン構築（通常パラメータ）
アプローチ: Label Encoding + LightGBM (5-Fold Stratified CV) + Charge_Difference特徴量
"""
...
def extract_features(train_df, test_df):
    """Label Encoding + Charge_Difference."""
...
# Training
# ============================================================================
def train_with_cv(X, y, cat_cols):
    print(f"\n{'='*80}")
...
def main():
    ...
    print(f"{'='*80}")
    X, X_test, cat_cols = extract_features(train_df, test_df)

    # Train
    models, oof_preds, fold_indices, fold_scores = train_with_cv(X, y, cat_cols)
