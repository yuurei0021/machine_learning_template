"""
Ridge OOF Generation (NB1 features, 20-fold)
Experiment: 20260328_05_ridge_nb1feat_oof

NB1 (BlamerX) FE pipeline -> Ridge prediction only.
Output: oof.csv, test_proba.csv for downstream stacking.
"""
...
def main():
    ...
    print("=" * 70)
    print("Ridge OOF Generation (NB1 features, 20-fold)")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        print(f"  Ridge AUC: {ridge_fold_auc:.6f}, Time: {(time.time()-t0)/60:.1f}min")

        del X_tr, X_val, X_te, ridge, X_tr_ridge, X_val_ridge, X_te_ridge
        gc.collect()

    # Results
    ridge_overall = roc_auc_score(train[CFG.TARGET], ridge_oof)
    ridge_ll = log_loss(train[CFG.TARGET].values, ridge_oof)

    print(f"\n{'='*70}")
    print(f"Ridge OOF AUC: {ridge_overall:.6f}, LogLoss: {ridge_ll:.6f}")
    print(f"Total time: {(time.time()-t0_all)/60:.1f}min")
    print(f"{'='*70}")

    # Save
    pd.DataFrame({"id": train_ids, "prob": ridge_oof, "predicted": (ridge_oof > 0.5).astype(int), "true": train[CFG.TARGET].values}).to_csv(CFG.PRED_DIR / "oof.csv", index=False)
    pd.DataFrame({"id": test_ids, "prob": ridge_pred}).to_csv(CFG.PRED_DIR / "test_proba.csv", index=False)
    pd.DataFrame({"id": test_ids, "Churn": ridge_pred}).to_csv(CFG.PRED_DIR / "test.csv", index=False)
