"""
Hill Climbing Ensemble: v4+++TT + KNN + SVM (Kaggle GPU)
- v4+++ model set (17 models) + TabTransformer + KNN(2) + SVM = 21 models
...
DATA_DIR_3 = Path("/kaggle/input/datasets/nyhalcyon/knn-svm-oof-predictions")
OUTPUT_DIR = Path("/kaggle/working")

# v4+++ models (17) + TabTransformer + KNN(2) + SVM = 21 models
MODELS = {
    ...
    "knn_baseline":             "20260331_09_knn_baseline",
    "knn_orig_ref":             "20260331_10_knn_orig_data_reference",
    "svm_orig_ref":             "20260331_11_svm_orig_data_reference",
}
...
def find_file(filename):
    for d in [DATA_DIR_1, DATA_DIR_2, DATA_DIR_3]:
        p = d / filename
        if p.exists():
            return p
    return None
...
def load_predictions():
    ...
    print(f"  DATA_DIR_3: {DATA_DIR_3} (exists={DATA_DIR_3.exists()})")

    # Debug: list contents
    for dd in [DATA_DIR_1, DATA_DIR_2, DATA_DIR_3]:
        if dd.exists():
...
def main():
    print("=" * 70)
    print("Hill Climbing: v4+++TT + KNN + SVM (21 models)")
    print("=" * 70)
