...
Experiment: 20260326_03_stacking_lr

Use OOF predictions from all models as features for a meta-learner (LR).
CV is applied on the stacking layer to avoid leakage.
"""
...
def main():
    ...
    print(f"\nOOF matrix: {oof_matrix.shape}, Test matrix: {test_matrix.shape}")

    # Stacking CV
    skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
    oof_preds = np.zeros(len(y_true))
    test_preds = np.zeros(len(test_matrix))

    for fold, (tr_idx, va_idx) in enumerate(skf.split(oof_matrix, y_true)):
        X_tr, X_val = oof_matrix[tr_idx], oof_matrix[va_idx]
        y_tr, y_val = y_true[tr_idx], y_true[va_idx]
        X_te = test_matrix
        ...
        if fold == 0:
            print(f"\n  LR coefficients:")
            for name, coef in sorted(zip(common_models, model.coef_[0]), key=lambda x: -abs(x[1])):
                print(f"    {name:<25s} {coef:+.4f}")
            print()

    oof_auc = roc_auc_score(y_true, oof_preds)
    oof_ll = log_loss(y_true, oof_preds)
