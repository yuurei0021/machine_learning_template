"""
Original Data Reference Distribution + LightGBM
Experiment: 20260325_01_lgbm_orig_data_reference

目的: 実験01の元データ参照特徴量をLightGBMに適用し、XGBoostとの比較・アンサンブル候補を作成
アプローチ: 9つの特徴量グループ + Nested Target Encoding + LightGBM (5-Fold Stratified CV)
ベース: 20260313_05_lightgbm_baseline (OOF AUC 0.9163)
特徴量: 20260323_01_orig_data_reference (OOF AUC 0.9185 with XGBoost)
"""
...
class Config:
    ...
    # LightGBM params (same as 05_lightgbm_baseline)
    LGB_PARAMS = {
        "objective": "binary",
...
# ============================================================================
# Feature Engineering (pre-CV: same as 20260323_01)
# ============================================================================
...
# ============================================================================
# Target Encoding (within CV fold, same as 20260323_01)
# ============================================================================
...
# ============================================================================
# Training (LightGBM with nested TE)
# ============================================================================
def train_with_cv(X, y, X_test_base, te_columns, ngram_columns):
    print(f"\n{'='*80}")
    print(f"Starting {config.N_SPLITS}-Fold Cross Validation (LightGBM)")
    print(f"{'='*80}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        test_preds_sum += model.predict(X_te, num_iteration=model.best_iteration)
...
def main():
    print("=" * 80)
    print("Original Data Reference Distribution + LightGBM")
    print("=" * 80)
    ...
    X_tr_tmp, X_val_tmp, X_te_tmp = apply_target_encoding(
        X.iloc[first_train_idx].copy(), y[first_train_idx],
        X.iloc[first_val_idx].copy(), X_test.copy(),
