...
Experiment: 20260323_02_xgb_optuna_tuning

目的: 実験01の元データ参照特徴量を使い、XGBoostのハイパーパラメータをOptunaで最適化
アプローチ: Phase1: Optuna探索(3-fold CV) → Phase2: 最適パラメータで5-fold CV + 予測
"""
...
class Config:
    ...
    # CV settings (same folds for search and final to avoid evaluation gap)
    SEARCH_N_SPLITS = 5
    FINAL_N_SPLITS = 5
...
def main(args):
    ...
    print("=" * 80)
    print("XGBoost Optuna Hyperparameter Tuning (with TE cache)")
    ...
    # ========================================================================
    # Pre-compute TE cache (one-time cost, reused by all trials + final)
    # ========================================================================
    print(f"\n{'='*80}")
    ...
    # Free original DataFrames to reclaim memory before Optuna search
    del train_df, test_df, orig_df, X, X_test
    import gc; gc.collect()
    print("  Freed source DataFrames from memory.")

    # ========================================================================
    # Phase 1: Optuna Search (using cached TE on disk)
    # ========================================================================
    print(f"\n{'='*80}")
    print(f"Phase 1: Optuna Search ({args.n_trials} trials)")
    print(f"{'='*80}")

    # Optuna study (in-memory, results saved to CSV)
    ...
    def log_callback(study, trial):
        # Save full trial history to CSV after each trial
        records = []
    ...
    # ========================================================================
    # Phase 2: Final Training with Best Params (reuses same TE cache)
    # ========================================================================
    best_params = study.best_params
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="XGBoost Optuna Tuning")
