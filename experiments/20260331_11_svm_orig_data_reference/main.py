"""
SVM Original Data Reference
Experiment: 20260331_11_svm_orig_data_reference

目的: 201特徴量でのSVM実装。マージンベースの決定境界でアンサンブル多様性を確保
アプローチ: Nystroem kernel approximation (RBF) + SGDClassifier (log_loss)
  - 594K行で標準SVC(RBF)はO(n^2)で非現実的 → Nystroem近似
  - Nystroem: RBFカーネルを有限次元空間に近似写像
  - SGDClassifier(log_loss): 近似カーネル空間でのロジスティック回帰 = 非線形決定境界 + calibrated probabilities
"""
...
class Config:
    ...
    # Nystroem params
    NYSTROEM_N_COMPONENTS = 500
    NYSTROEM_KERNEL = "rbf"
    NYSTROEM_GAMMA = "scale"
...
def add_pre_cv_features(train_df, test_df, orig_df):
    ...
    # Group 2: Arithmetic Interactions
    ...
    # Group 7: Quantile Distance Features
    ...
    # Group 8: Digit/Modular Features
...
def ohe_categoricals(X_tr, X_val, X_te, cat_cols):
    """Label Encoded カテゴリカルをOHEに変換（距離/カーネルベースモデル用）"""
...
def train_with_cv(X, y, X_test_base, te_columns, ngram_columns):
    print(f"\n{'='*80}")
    print(f"Starting {config.N_SPLITS}-Fold CV (Nystroem RBF + SGD, n_components={config.NYSTROEM_N_COMPONENTS})")
    print(f"{'='*80}")
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        # gamma='scale' equivalent: 1 / (n_features * X.var())
        gamma_val = 1.0 / (X_tr_scaled.shape[1] * X_tr_scaled.var())
        print(f"  Nystroem transform (n_components={config.NYSTROEM_N_COMPONENTS}, gamma={gamma_val:.6f})...")
        nystroem = Nystroem(
            kernel=config.NYSTROEM_KERNEL,
            gamma=gamma_val,
            n_components=config.NYSTROEM_N_COMPONENTS,
            random_state=config.RANDOM_STATE + fold,
        )
        X_tr_kernel = nystroem.fit_transform(X_tr_scaled)
        X_val_kernel = nystroem.transform(X_val_scaled)
        X_te_kernel = nystroem.transform(X_te_scaled)
        ...
        sgd.fit(X_tr_kernel, y_tr)

        val_proba = sgd.predict_proba(X_val_kernel)[:, 1]
        oof_preds[val_idx] = val_proba

        test_proba = sgd.predict_proba(X_te_kernel)[:, 1]
...
def main():
    print("=" * 80)
    print("SVM Original Data Reference (Nystroem RBF + SGD)")
    print("=" * 80)
