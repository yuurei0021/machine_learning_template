...
Experiment: 20260326_02_hillclimb_ensemble

Greedy hill climbing: start from the best single model, iteratively add
models with the weight that maximizes AUC-ROC. Negative weights allowed.
"""
...
def apply_weights_to_test(test_dict, used_models):
    """Reconstruct test blend by replaying the hill climbing weight history."""
    model_order = list(used_models.keys())

    # Start from the first model (weight=1.0 initially, then scaled)
    first_model = model_order[0]
    test_ensemble = test_dict[first_model].copy()

    # We need to replay the weight application steps.
    # used_models stores final weights after all scaling.
    # Instead, just do a weighted sum using final weights.
    ...
    # Normalize if weights don't sum to 1 (they should be close)
