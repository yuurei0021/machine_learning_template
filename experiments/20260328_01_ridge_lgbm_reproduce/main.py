"""
Ridge -> LightGBM Two-Stage (NB1 structure with LGBM)
Experiment: 20260328_01_ridge_lgbm_reproduce

NB1 (BlamerX) Ridge->XGB pipeline with XGB replaced by LightGBM Optuna params.
20-fold CV, same FE as NB1.
"""
...
# LightGBM Parameters (from 20260325_02 Optuna)
LGB_PARAMS = {
    "objective": "binary",
...
def main():
    ...
    print("=" * 70)
    print("Ridge -> LightGBM Two-Stage (NB1 structure)")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        print(f"  Ridge AUC: {ridge_fold_auc:.6f}, LGB AUC: {fold_auc:.6f}, Best iter: {model.best_iteration}, Time: {(time.time()-t0)/60:.1f}min")
    ...
    print(f"LGB   OOF AUC: {xgb_overall:.6f} (mean fold: {np.mean(xgb_fold_scores):.6f})")
