"""
Ridge → XGB DART Two-Stage
Experiment: 20260401_02_ridge_xgb_dart

ridge_xgb_reproduce (26_04) のboosterをdartに変更。
XGBoost DARTはDropout正則化により通常gbtreeと異なる誤差パターンを生成。
LGB DART版が HC v4TT+DART でw=-0.172でアンサンブル貢献したため、XGB版でも検証。
"""
...
# XGBoost DART Parameters (from original notebook, booster changed to dart)
# gbdt版のbest_iterationは概ね3000-5000だったため、4000に固定
XGB_PARAMS = {
    "n_estimators": 4000,
    "booster": "dart",
...
def main():
    ...
    print("=" * 70)
    print("Ridge -> XGB DART Two-Stage")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # DART: no early stopping, fixed n_estimators with validation monitoring
        model = xgb.XGBClassifier(**XGB_PARAMS)
        model.fit(X_tr, y_tr, eval_set=[(X_val, y_val)], verbose=1000)

        # DART prediction: use ntree_limit=0 to use all trees
        ...
        print(f"  Ridge AUC: {ridge_fold_auc:.6f}, XGB-DART AUC: {fold_auc:.6f}, Rounds: {XGB_PARAMS['n_estimators']}, Time: {(time.time()-t0)/60:.1f}min")
