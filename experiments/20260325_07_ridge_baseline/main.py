...
Experiment: 20260325_07_ridge_baseline

目的: One-Hot Encoding + StandardScaler + Ridge Regression（追加特徴量なし）
Ridge回帰で0/1ターゲットを直接予測し、出力をclipして確率とする
"""
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        X_tr_s = scaler.fit_transform(X.iloc[tr_idx])
        X_va_s = scaler.transform(X.iloc[va_idx])
        ...
        test_preds += np.clip(model.predict(X_te_s), 0, 1) / config.N_SPLITS
    ...
    print(f"OOF LogLoss: {log_loss(y, np.clip(oof_preds, 1e-7, 1-1e-7)):.6f}")
