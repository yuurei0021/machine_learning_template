"""
Ridge -> LightGBM DART Two-Stage
Experiment: 20260331_13_ridge_lgbm_dart

ridge_lgbm_reproduce (28_01) のboosting_typeをdartに変更。
DARTはDropout正則化によりgbdtと異なる誤差パターンを生成し、アンサンブル多様性に貢献する。
"""
...
# LightGBM DART Parameters (from 20260325_02 Optuna, boosting_type changed to dart)
LGB_PARAMS = {
    "objective": "binary",
    ...
    "subsample": 0.7135,
    "subsample_freq": 1,
    "colsample_bytree": 0.3838,
    "reg_alpha": 3.9529,
    "reg_lambda": 0.009362,
    "min_split_gain": 0.5676,
    "drop_rate": 0.1,
    "skip_drop": 0.5,
...
# DART does not support early stopping; use fixed rounds
# gbdt版のbest_iterationは概ね600-900だったため、800に固定
NUM_BOOST_ROUND = 800
...
def main():
    ...
    print("=" * 70)
    print("Ridge -> LightGBM DART Two-Stage")
    ...
    for i, (train_idx, val_idx) in enumerate(skf_outer.split(train, train[CFG.TARGET])):
        ...
        # === STAGE 2: LightGBM with Ridge predictions ===
        ...
        # LightGBM uses categorical_feature, drop raw cat columns from features
        for df in [X_tr, X_val, X_te]:
        ...
        COLS_LGB = X_tr.columns.tolist()

        if i == 0:
            print(f"  LGB features: {len(COLS_LGB)}")

        dtrain = lgb.Dataset(X_tr, label=y_tr)
        dval = lgb.Dataset(X_val, label=y_val, reference=dtrain)

        # DART: no early stopping, fixed rounds with validation monitoring
        model = lgb.train(
            LGB_PARAMS,
            dtrain,
        ...
        xgb_pred += model.predict(X_te[COLS_LGB]) / CFG.N_FOLDS

        print(f"  Ridge AUC: {ridge_fold_auc:.6f}, LGB-DART AUC: {fold_auc:.6f}, Rounds: {NUM_BOOST_ROUND}, Time: {(time.time()-t0)/60:.1f}min")
    ...
    print(f"DART  OOF AUC: {xgb_overall:.6f} (mean fold: {np.mean(xgb_fold_scores):.6f})")
