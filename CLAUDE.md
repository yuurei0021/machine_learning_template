...
├── .gitignore              # Git除外設定
├── CLAUDE.md               # このファイル
└── REPORT.md               # モデル精度一覧・アンサンブル結果・知見レポート
```
...
- **一時ファイルの管理**: 一時ファイルやスクラッチファイルはプロジェクトルートに作成せず、必ず該当実験ディレクトリ内に配置すること。リポジトリルートの汚染を禁止する
- **README.md記入ルール**: 実験実行後は必ず各実験の `README.md` に結果を記入すること（目的、アプローチ、パラメータ、結果スコア、考察、次のステップ）。README未記入のまま次の実験に進まない
- **REPORT.md更新ルール**: 実験完了後、リポジトリトップの `REPORT.md` にモデル精度（OOF AUC, LB）を追記すること。単体モデル・アンサンブルそれぞれ該当テーブルに行を追加し、LBスコア推移にも記録する
